...
echo '**** PUSHING covidapi-codelifter-functions'
container_name=covidapi-codelifter-functions
docker commit -m "A An Azure Function implementation of the CodeLifter Covid Data API as of ${fileData}" -a "Andrew Palmer (CodeLifterIO)" ${container_name} codelifterio/${container_name}:${fileData} 
docker push codelifterio/${container_name}:latest
docker push codelifterio/${container_name}:${fileData} 
...
echo '**** PUSHING covidapi-codelifter-redis'
container_name=covidapi-codelifter-redis
docker commit -m "A working Redis container" -a "Andrew Palmer (CodeLifterIO)" ${container_name} codelifterio/${container_name}:${fileData} 
docker push codelifterio/${container_name}:latest
docker push codelifterio/${container_name}:${fileData} 
